...
*/
matrix_t* kalman_get_state_vector(kalman_t *kf)
{
    return &(kf->x);
}
...
*/
matrix_t* kalman_get_state_transition(kalman_t *kf)
{
    return &(kf->A);
}
...
*/
matrix_t* kalman_get_system_covariance(kalman_t *kf)
{
    return &(kf->P);
}
...
*/
matrix_t* kalman_get_input_vector(kalman_t *kf)
{
    return &(kf->u);
}
...
*/
matrix_t* kalman_get_input_transition(kalman_t *kf)
{
    return &(kf->B);
}
...
*/
matrix_t* kalman_get_measurement_vector(kalman_measurement_t *kfm)
{
    return &(kfm->z);
}
...
*/
matrix_t* kalman_get_measurement_transformation(kalman_measurement_t *kfm)
{
    return &(kfm->H);
}
...
*/
matrix_t* kalman_get_process_noise(kalman_measurement_t *kfm)
{
    return &(kfm->R);
}
...
void kalman_predict_x(register kalman_t *const kf)
{
    ...
    /* x = A*x                                                              */
    /* should have u input as well: x_k = A*x_k-1 + B*u_k-1                 */
    /* ref: https://www.cs.unc.edu/~welch/media/pdf/kalman_intro.pdf        */
    /************************************************************************/
...
void kalman_predict_Q(register kalman_t *const kf)
{
    ...
    /* P = A*P*A' + B*Q*B' - this seems wrong                               */
    /* P = A*P*A' + Q - this seems right                                    */
    /* where is x_k = A*x_k-1 + B*u_k-1 ?                                   */
    /* ref: https://www.cs.unc.edu/~welch/media/pdf/kalman_intro.pdf        */
    /************************************************************************/
    ...
    matrix_add_inplace(P, &kf->Q); // what was missing before

    // P = P + B*Q*B'
    //if (kf->B.rows > 0) // this most likely wrong
    //{
